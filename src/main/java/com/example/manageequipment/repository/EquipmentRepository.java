...
//    @Query("SELECT e from Equipment e where e.name like %:name%")
    ...
    @Query(value = "SELECT e.* FROM equipment e WHERE owner_id = :ownerId and NAME LIKE %:name%",
            countQuery = "SELECT count(e.*) FROM equipment e WHERE owner_id = :ownerId and NAME LIKE %:name%",
            nativeQuery = true)
